...
if [[ "${1:-}" == "--restore" ]]; then
  ...
  # Expect '--to <target>'
  if [[ "${1:-}" != "--to" ]]; then
    echo "Usage: $0 --restore <backup-file> --to <restore-directory>"
    exit 1
  fi
  shift
  ...
  # Expand ~ properly
  BACKUP_FILE="$(eval echo "$BACKUP_FILE")"
  RESTORE_TARGET="$(eval echo "$RESTORE_TARGET")"
  ...
  if [[ "$DRYRUN" == "true" ]]; then
    ...
    tar --strip-components=1 -xzf "$BACKUP_FILE" -C "$RESTORE_TARGET"
    if [[ $? -eq 0 ]]; then
      log "SUCCESS: Restore completed successfully!"
    else
      log "ERROR: Restore failed during extraction!"
      exit 1
    fi
  fi
